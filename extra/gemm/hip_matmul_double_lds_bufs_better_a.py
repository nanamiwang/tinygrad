...
extern "C" __global__ void __launch_bounds__ (32, 1) test(float* c, __half* a, __half* b) {{
  ...
  // KX&KY must be >= 2
  // Double buffers
  __shared__ half a_lds[16*{KX}*2][32];
  __shared__ half b_lds[16*{KY}*2][32];
  ...
  // Prefetch for k = 0
  for (int row = 0; row < {KY}*16; row++) {{
    a_gr[row] = a[0+threadIdx.x + {N}*row];
  }}
  ...
  for (int row = 0; row < {KY}*16; row++) {{
    a_lds_ptr_0[row][threadIdx.x] = a_gr[row];
  }}
  ...
  for (int k = 0; k < {N-32}; k += 32) {{
    __syncthreads();
    // Prefetch for k+32
    for (int row = 0; row < {KY}*16; row++) {{
      a_gr[row] = a[k+32+threadIdx.x + {N}*row];
    }}

    for(int kk = 0; kk < {KY}/2;kk++) {{
      for (int ele = 0; ele < 32; ++ele) {{
        b_gr[kk][ele] = b[(k+32+ele)*{N} + (threadIdx.x/16 + 2*kk)*16 + threadIdx.x%16];
      }}
    }}
    ...
    for (int y = 0; y < {KY}; y++) {{
      for (int x = 0; x < {KX}; x++) {{
        for (int ele = 0; ele < 16; ++ele) {{
          a_frag[ele] = a_lds_ptr_0[x*16 + lane][ele+16];
        }}

        for (int ele = 0; ele < 16; ++ele) {{
          b_frag[ele] = b_lds_ptr_0[y*16 + lane][ele+16];
        }}
    ...
    for (int row = 0; row < {KY}*16; row++) {{
      a_lds_ptr_1[row][threadIdx.x] = a_gr[row];
    }}
