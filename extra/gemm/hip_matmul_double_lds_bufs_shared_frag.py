...
extern "C" __global__ void __launch_bounds__ (32, 1) test(float* c, __half* a, __half* b) {{
  ...
  __shared__ half16 a_frag[32];
  __shared__ half16 b_frag[32];
  #ifdef F32
    float8 c_frag[{KY}][{KX}] = {{}};
  #else
    half16 c_frag[{KY}][{KX}] = {{}};
  ...
  for (int k = 0; k < {N-16}; k += 16) {{
    ...
    for (int y = 0; y < {KY}; y++) {{
      for (int x = 0; x < {KX}; x++) {{
        for (int ele = 0; ele < 16; ++ele) {{
          a_frag[threadIdx.x][ele] = a_lds_ptr_0[x*16 + lane][ele];
        }}

        for (int ele = 0; ele < 16; ++ele) {{
          b_frag[threadIdx.x][ele] = b_lds_ptr_0[y*16 + lane][ele];
        }}
        half16 a_frag_local = a_frag[threadIdx.x];
        half16 b_frag_local = b_frag[threadIdx.x];
        #ifdef F32
          c_frag[y][x] = __builtin_amdgcn_wmma_f32_16x16x16_f16_w32(a_frag_local, b_frag_local, c_frag[y][x]);
        #else
          c_frag[y][x] = __builtin_amdgcn_wmma_f16_16x16x16_f16_w32(a_frag_local, b_frag_local, c_frag[y][x], false);
        #endif
      }}
    }}
  ...
  for (int y = 0; y < {KY}; y++) {{
    for (int x = 0; x < {KX}; x++) {{
      ...
      #ifdef F32
        c_frag[y][x] = __builtin_amdgcn_wmma_f32_16x16x16_f16_w32(a_frag[threadIdx.x], b_frag[threadIdx.x], c_frag[y][x]);
      #else
