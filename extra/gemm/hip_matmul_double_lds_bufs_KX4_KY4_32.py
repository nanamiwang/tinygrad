...
extern "C" __global__ void __launch_bounds__ (32, 1) test(float* c, __half* a, __half* b) {{
  ...
  for (int k = 0; k < {N-16}; k += 16) {{
    ...
    for (int y = 0; y < 4; y++) {{
      for (int x = 0; x < 4; x++) {{
        a_lds_ptr_1[threadIdx.x+32*0][y*4+x] = a_gr[0][y*4+x];
        b_lds_ptr_1[threadIdx.x+32*0][y*4+x] = b_gr[0][y*4+x];
        a_lds_ptr_1[threadIdx.x+32*1][y*4+x] = a_gr[1][y*4+x];
        b_lds_ptr_1[threadIdx.x+32*1][y*4+x] = b_gr[1][y*4+x];
      }}
    }}
